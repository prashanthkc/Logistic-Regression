...
# Load the Dataset
ads <- read.csv(file.choose()) # Choose the advtisement Data set
View(ads)
sum(is.na(ads))
...
colnames(claimants)
ads <- ads[ , -c(5,6,8,9)] # Removing the first column which is is an Index

attach(ads)
# Preparing a linear regression 
mod_lm <- lm(Clicked_on_Ad ~ ., data =ads)
summary(mod_lm)

pred1 <- predict(mod_lm, ads)
pred1
# plot(claimants$CLMINSUR, pred1)
...
# Confusion matrix and considering the threshold value as 0.5 
confusion <- table(prob > 0.5, ads$Clicked_on_Ad)
confusion
...
################################ PROBLEM 3 ###########################################
# Load the Dataset
election_data <- read.csv(file.choose()) # Choose the election Data set
View(election_data)
sum(is.na(ads))
...
election_data <- election_data[-1 , -1] # Removing the first column which is is an Index

attach(election_data)
# Preparing a linear regression 
mod_lm <- lm(Result ~ ., data =election_data)
summary(mod_lm)

pred1 <- predict(mod_lm, election_data)
pred1
# plot(claimants$CLMINSUR, pred1)
...
# Load the Dataset
bank_data <- read.csv(file.choose()) # Choose the bank_data Data set
View(bank_data)
sum(is.na(bank_data))
